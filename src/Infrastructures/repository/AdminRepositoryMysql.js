...
class AdminRepositoryMysql extends AdminRepository{
    ...
    async auth({username_hash,password_hash}){
        ...
        const [[admin]] = await this._pool.execute("SELECT nama,id FROM admins WHERE username_hash = ? AND password_hash = ?",values)
        return admin || null
    }
    async update({username_hash,password_hash,nama,id}){
        
        const values = [username_hash,password_hash,nama,id]
        await this._pool.execute("UPDATE admins SET username_hash = ?, password_hash =? ,nama=? WHERE id=?",values)
        return id
    }
    ...
    async addNewAdmin({username_hash,password_hash,nama}){
        ...
        const values = [username_hash,password_hash,nama,id]
        await this._pool.execute("INSERT INTO admins (username_hash,password_hash,nama,id) VALUES (?,?,?,?)",values)
        return id
