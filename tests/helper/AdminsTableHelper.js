...
class AdminsTableHelper {
   ...
   async addAdmin({
    ...
    await this._pool.execute("INSERT INTO admins (id,nama,username_hash,password_hash) VALUES (?,?,?,?)",values)
    }
    async clear(){
        await this._pool.query("DELETE FROM admins WHERE 1=1")
    }
    async exists(adminId){
        const [[{t}]] = await this._pool.execute("SELECT COUNT(*) AS t FROM admins WHERE id = ?",[adminId])
        return t == 1
    }
    async detail(adminId){
        const [[admin]] = await this._pool.execute("SELECT * FROM admins WHERE id=?",[adminId])
        return admin
    }
}
